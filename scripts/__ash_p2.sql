...
SELECT s_tot.sql_plan_hash_value plan_hash_value,
       ...
       case when summ.event is null and summ.smpl_cnt is not null then 'CPU' else summ.event
       end event,
       summ.smpl_cnt * 10 tim,
       round(100 * summ.smpl_cnt / s_tot.smpl_cnt, 2) tim_pct,
	   to_char(summ.start_tim, 'yyyy/mm/dd hh24:mi:ss') step_start, 
	   to_char(summ.end_tim, 'yyyy/mm/dd hh24:mi:ss') step_end
